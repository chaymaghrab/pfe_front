...
@Component({
  selector: 'app-groupe-certif',
  templateUrl: './groupe-certif.component.html',
  styleUrls: ['./groupe-certif.component.css']
})
export class GroupeCertifComponent implements OnInit {

  public groupecertiftp_add: {
    nom_groupe_certif: string;
  ...
  public affacter_etd_grp_certif_tp: {
    grp_certif_id: number;
  ...
  ngOnInit(): void {
    ...
    this.groupecertif = this.fb.group({
  ...
  recherche_grp() {
    ...
    this.service_grp_certification.get_etud_filtre(this.cherchergroupe.value)
      .subscribe((data: any) => {
      this.taillesousgroupe=data.length;
        this.rows = data;
        this.list_etudiant=data;
      ...
      Swal.fire({
        ...
        if (result.value) {
          while (x - incr >= y) {
            ...
            this.indice_groupe++;
            this.groupecertiftp.push(this.list_etd_groupe);
            console.log(this.groupecertiftp);
              ...
              if (t > 0) {
                for (let p = 0; p < Math.floor(x / y) - 1; p++) {
                  for (let n = incr; n < incr + t; n++) {
                    this.groupecertiftp[p].push(this.list_etudiant[n]);
                  }
                  incr += t;
                }
                for (let m = incr; m < x; m++) {
                  this.groupecertiftp[Math.floor(x / y) - 1].push(this.list_etudiant[m])
                }
                ...
                for (let z = incr; z < x; z++) {
                  this.groupecertiftp[o].push(this.list_etudiant[z])
                  o++
                }
              }
            }
          ...
          console.log(this.groupecertiftp);
          this.groupecertiftp = [...this.groupecertiftp];
  ...
  save_grp_certif() {
    ...
    this.service_grp_certification.get_grp_certif_byfiltre(this.cherchergroupe.value).subscribe((rs: any) => {
      ...
      if (b == true) {
        rs.forEach(rs_elem => {
          this.service_grp_certification.delete_grp_certif(rs_elem.id).subscribe(resul => {
            console.log(resul);
          })
        });
        this.groupecertiftp.forEach(element => {
          var list_etd_id = [];
          // console.log(element)
          this.groupecertiftp_add = {
            nom_groupe_certif: element.name,
            certification_id: element.certif,
            langue: element.langue,
            cours: element.cours,
            effectif: element.effectif
          }
          console.log(this.groupecertiftp_add);
          this.service_grp_certification.add_grp_certif(this.groupecertiftp_add).subscribe((data: any) => {
            console.log(data);
            element.forEach((elem: any) => {
              list_etd_id.push(elem.id);
            });
            this.affacter_etd_grp_certif_tp = {
              grp_certif_id: data.id,
              etudiant_id: list_etd_id,
            }
            console.log(this.affacter_etd_grp_certif_tp)
            this.service_grp_certification.affecter_etd_grp_certif(this.affacter_etd_grp_certif_tp).subscribe((res: any) => {
              console.log(res);
            });
     ...
     console.log(data);
      ...
      data.forEach((element: any) => {
        ...
        this.service_grp_certification.get_etudiants(element.id).subscribe((res: any) => {
          ...
          this.groupecertiftp.push(res);
          console.log(this.groupecertiftp);
          this.groupecertiftp = [...this.groupecertiftp];
          this.selected = [this.groupecertiftp[0]];
        });
